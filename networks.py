...
class GenZ(nn.Module):
    def __init__(self, input_size=2048, h1=1024, h2=512, normalize=True):
        ...
        self.layer2 = nn.Sequential(nn.Linear(h1, h2), nn.ReLU())
...
class CrossDomainNetwork(nn.Module):  
    def __init__(self):
        ...
        self.feature_extractor = GenZ()
        self.classifier = nn.Linear(512, 17)
...
class TargetDomainNetwork(nn.Module):
    def __init__(self):
        super(TargetDomainNetwork, self).__init__()
        self.feature_extractor = GenZ()
        self.classifier = nn.Linear(512, 17)
